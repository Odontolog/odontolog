...
public interface TreatmentPlanApi {
    ...
    @Operation(summary = "Assign user to treatment plan", description = """
            ...
            Postconditions: The treatment plan will be assigned to the specified user.""")
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "Treatment Plan is not in a state that allows user assignment.")
    })
    public ResponseEntity<TreatmentPlanDTO> assignUserToTreatmentPlan(
            @RequestBody TreatmentPlanAssignUserRequestDTO requestDTO,
            @Parameter Long treatment_id);
}
