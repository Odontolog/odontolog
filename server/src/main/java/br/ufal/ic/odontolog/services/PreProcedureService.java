...
public class PreProcedureService {
  ...
  public PreProcedureDTO getPreProcedureByPatientAndId(Long patientId, Long preProcedureId) {
    ...
    PreProcedure preProcedure =
        preProcedureRepository
            .findByIdAndPatient(preProcedureId, patient)
            .orElseThrow(
                () -> new ResourceNotFoundException("PreProcedure not found for this patient"));
  ...
  public List<PreProcedureShortDTO> getPreProceduresForPatient(Long patientId) {
    List<PreProcedure> procedures = preProcedureRepository.findByPatientId(patientId);

    return procedures.stream().map(preProcedureMapper::toShortDTO).collect(Collectors.toList());
  }
}
