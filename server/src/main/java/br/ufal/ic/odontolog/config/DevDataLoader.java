...
public class DevDataLoader implements CommandLineRunner {
    // TODO: We must add a Logger here
    ...
    public DevDataLoader(StudentRepository studentRepository, SupervisorRepository supervisorRepository, TreatmentPlanRepository treatmentPlanRepository, AttachmentRepository attachmentRepository, PreProcedureRepository preProcedureRepository) {
    ...
    public void run(String... args) throws Exception {
        ...
        TreatmentPlan treatmentPlanTest001 = treatmentPlanRepository.save(new TreatmentPlan(
        ...
        if (treatmentPlanTest001.getProcedures().stream().findFirst().isPresent()) {
            treatmentPlanTest001.getProcedures().stream().findFirst().get().getAttachments().add(attachmentTest001);
            treatmentPlanTest001 = treatmentPlanRepository.save(treatmentPlanTest001);
        } else {
            System.out.println("No procedures found"); // TODO: We must add a Logger here
        }
