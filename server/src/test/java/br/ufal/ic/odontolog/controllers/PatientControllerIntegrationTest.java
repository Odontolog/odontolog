...
public class PatientControllerIntegrationTest {
        ...
        void uploadAttachment_uploadsFileAndReturnsUrl() throws Exception {
                String jsonInitUpload = mockMvc.perform(
                                MockMvcRequestBuilders.multipart("/api/patients/{id}/attachments/init-upload",
                                                patient.getId()))
                                .andExpect(MockMvcResultMatchers.status().isOk())
                                .andExpect(
                                                MockMvcResultMatchers.jsonPath("$.uploadUrl")
                                                                .isNotEmpty())
                                .andExpect(
                                                MockMvcResultMatchers.jsonPath("$.objectKey")
                                                                .isNotEmpty())
                                .andReturn()
                                .getResponse()
                                .getContentAsString();

                String uploadUrl = objectMapper
                                .readTree(jsonInitUpload)
                                .get("uploadUrl")
                                .asText();
                URI uploadUri = URI.create(uploadUrl);
                String objectKey = objectMapper
                                .readTree(jsonInitUpload)
                                .get("objectKey")
                                .asText();
                ...
                boolean doesObjectExist = s3Template.objectExists(
                ...
                String createAttachmentResponse = mockMvc
                                .perform(MockMvcRequestBuilders
                                                .post("/api/patients/%s/attachments".formatted(patient.getId()))
                ...
                Long attachmentId = objectMapper
                                .readTree(createAttachmentResponse)
                                .get("id")
                                .asLong();
                String getAttachmentJson = mockMvc.perform(
