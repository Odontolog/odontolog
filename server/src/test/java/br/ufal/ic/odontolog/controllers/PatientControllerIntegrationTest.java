...
public class PatientControllerIntegrationTest {
  ...
  void uploadAttachment_uploadsFileAndReturnsUrl() throws Exception {
    ...
    boolean doesObjectExist =
        s3Template.objectExists(s3Properties.getBuckets().getPrivateBucket(), objectKey);
    ...
    String createAttachmentResponse =
        mockMvc
            .perform(
                MockMvcRequestBuilders.post(
                        "/api/patients/%s/attachments".formatted(patient.getId()))
    ...
    ResponseEntity<byte[]> downloadResponse =
        restTemplate.exchange(downloadUri, HttpMethod.GET, null, byte[].class);
  ...
  void createAttachment_fileNotInS3_returnsUnprocessableEntity() throws Exception {
    ...
    mockMvc
        .perform(
            MockMvcRequestBuilders.post("/api/patients/%s/attachments".formatted(patient.getId()))
                .contentType(MediaType.APPLICATION_JSON)
                .content(createAttachmentRequest))
